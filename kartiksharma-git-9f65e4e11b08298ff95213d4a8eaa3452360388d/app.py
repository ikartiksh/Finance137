...
def index():
    ...
    return render_template("index.html", stocks=stocks, cahs=cash, total_value=total_value, grand_total=grand_total)
...
def buy():
    ...
    if request.method == "POST":
        ...
        db.execute("INSERT into transactions(user_id, shares,symbol,price) VALUES(:user_id, :symbol, :shares, :price)",
...
def register():
    ...
    if request.method == "POST":
        ...
        if not request.form.get("username"):
            ...
            return apology("must confirm", 400)
...
def sell():
    ...
    if request.method == "POST":
        ...
        if not symbol:
            ...
            return apology("must provide a positive no of integers")
        else:
            shares = int(shares)
